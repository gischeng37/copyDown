function copyDown_getInstitutionalTrackerObject() {
  ...
  if (!(institutionalTrackingString)||(institutionalTrackingString=='')) {
    copyDown_institutionalTrackingUi();
    return;
  }
}


function copyDown_institutionalTrackingUi() {
  ...
  var checkHandler = app.createServerHandler('copyDown_refreshTrackingGrid').addCallbackElement(panel);
  ...
  var saveHandler = app.createServerHandler('copyDown_saveInstitutionalTrackingInfo').addCallbackElement(panel);
...
function copyDown_refreshTrackingGrid(e) {
...
function copyDown_saveInstitutionalTrackingInfo(e) {
  var app = UiApp.getActiveApplication();
...
function copyDown_createInstitutionalTrackingUrls(institutionTrackingObject, encoded_page_name, encoded_script_name) {
  for (var key in institutionTrackingObject) {
   var utmcc = copyDown_createGACookie();
  if (utmcc == null)
    {
      return null;
    }
  ...
  var ga_url1 = "http://www.google-analytics.com/__utm.gif?utmwv=5.2.2&utmhn=www.copyDown-analytics.com&utmcs=-&utmul=en-us&utmje=1&utmdt&utmr=0=";
...
function copyDown_createGATrackingUrl(encoded_page_name)
{
  var utmcc = copyDown_createGACookie();
  var eduSetting = UserProperties.getProperty('eduSetting');
   if (eduSetting=="true") {
  ...
  var ga_url1 = "http://www.google-analytics.com/__utm.gif?utmwv=5.2.2&utmhn=www.copyDown-analytics.com&utmcs=-&utmul=en-us&utmje=1&utmdt&utmr=0=";
  var ga_url2 = "&utmac=UA-31030730-1&utmcc=" + utmcc + "&utmu=DI~";
  var ga_url_full = ga_url1 + encoded_page_name + "&utmp=" + encoded_page_name + ga_url2;
...
function copyDown_createSystemTrackingUrls(institutionTrackingObject, encoded_system_name, encoded_execution_name) {
  for (var key in institutionTrackingObject) {
  var utmcc = copyDown_createGACookie();
  if (utmcc == null)
    {
...
function copyDown_createGACookie()
{
  var a = "";
  var b = "100000000";
  var c = "200000000";
  ...
  if ((copyDown_uid == null) || (copyDown_uid == ""))
    {
      // shouldn't happen unless user explicitly removed copyDown_uid from properties.
      return null;
    }
  
  a = copyDown_uid.substring(0,9);
  d = copyDown_uid.substring(9);
...
{
  var ga_url = copyDown_createGATrackingUrl("Formulas%20Copied%20Down");
  if (ga_url)
    {
      var response = UrlFetchApp.fetch(ga_url);
    }
  var institutionalTrackingObject = copyDown_getInstitutionalTrackerObject();
  if (institutionalTrackingObject) {
    copyDown_createInstitutionalTrackingUrls(institutionalTrackingObject,"Formulas%20Copied%20Down", "copyDown");
    var systemName = ScriptProperties.getProperty('systemName');
    if (systemName) {
      var encoded_system_name = urlencode(systemName);
      copyDown_createSystemTrackingUrls(institutionalTrackingObject, encoded_system_name, "Formulas%20Copied%20Down")
    }
  }
}
...
{
  var ga_url = copyDown_createGATrackingUrl("First%20Install");
  if (ga_url)
    {
      var response = UrlFetchApp.fetch(ga_url);
    }
    var institutionalTrackingObject = copyDown_getInstitutionalTrackerObject();
  if (institutionalTrackingObject) {
    copyDown_createInstitutionalTrackingUrls(institutionalTrackingObject,"First%20Install", "copyDown");
    var systemName = ScriptProperties.getProperty('systemName');
    if (systemName) {
      var encoded_system_name = urlencode(systemName);
      copyDown_createSystemTrackingUrls(institutionalTrackingObject, encoded_system_name, "First%20Install")
    }
  }
}
...
{
  var ga_url = copyDown_createGATrackingUrl("Repeat%20Install");
  if (ga_url)
    {
      var response = UrlFetchApp.fetch(ga_url);
    }
      var institutionalTrackingObject = copyDown_getInstitutionalTrackerObject();
  if (institutionalTrackingObject) {
    copyDown_createInstitutionalTrackingUrls(institutionalTrackingObject,"Repeat%20Install", "copyDown");
    var systemName = ScriptProperties.getProperty('systemName');
    if (systemName) {
      var encoded_system_name = urlencode(systemName);
      copyDown_createSystemTrackingUrls(institutionalTrackingObject, encoded_system_name, "Repeat%20Install")
    }
  }
}


function setcopyDownUid()
{ 
  var copyDown_uid = UserProperties.getProperty("copyDown_uid");
  if (copyDown_uid == null || copyDown_uid == "")
    {
      // user has never installed copyDown before (in any spreadsheet)
      var dt = new Date();
      var ms = dt.getTime();
      var ms_str = ms.toString();
 
      UserProperties.setProperty("copyDown_uid", ms_str);
